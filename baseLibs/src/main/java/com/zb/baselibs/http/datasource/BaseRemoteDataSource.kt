...
import com.zb.baselibs.http.callback.BaseRequestCallback
import com.zb.baselibs.http.coroutine.ICoroutineEvent
import com.zb.baselibs.http.exception.BaseHttpException
import com.zb.baselibs.http.exception.LocalBadException
...
abstract class BaseRemoteDataSource<Api : Any>(
    ...
    protected fun handleException(throwable: Throwable, callback: BaseRequestCallback?) {
        ...
        if (exceptionHandle(exception)) {
            ...
            if (callback.onFailToast()) {
                ...
                if (error.isNotBlank()) {
                    Toast.makeText(BaseApp.context, error, Toast.LENGTH_SHORT).show()
                }
            }
        }
    }

    private fun generateBaseExceptionReal(throwable: Throwable): BaseHttpException {
    ...
    protected fun showLoading(job: Job?, msg: String?) {
        iUiActionEvent?.showLoading(job, msg)
    }
